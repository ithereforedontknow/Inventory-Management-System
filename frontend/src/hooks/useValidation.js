...
export function useValidation(rules) {
  ...
  return { errors, validate, setFieldError, clearField, clearErrors, applyServerErrors }
}

// ── Common rule factories ─────────────────────────────────────────────────────

export const rules = {
  required: (label) => (v) => (!v && v !== 0) || String(v).trim() === '' ? `${label} is required` : null,

  minLength: (min, label) => (v) => v && String(v).trim().length < min ? `${label} must be at least ${min} characters` : null,

  maxLength: (max, label) => (v) => v && String(v).length > max ? `${label} must be no more than ${max} characters` : null,
  ...
  nonNegativeInt: (label) => (v) => {
    if (v === '' || v === null || v === undefined) return null
    const n = Number(v)
    if (!Number.isInteger(n) || n < 0) return `${label} must be 0 or a positive whole number`
    return null
  },
  ...
  oneOf: (options, label) => (v) => {
    if (!v) return null
    return options.includes(v) ? null : `${label} must be one of: ${options.join(', ')}`
  },
